...
const MAX_PDF_OCR_PAGES = Number.parseInt(process.env.MAX_PDF_OCR_PAGES || "3", 10);
const PDF_OCR_DESIRED_WIDTH = Number.parseInt(process.env.PDF_OCR_DESIRED_WIDTH || "1200", 10);
const MAX_PDF_OCR_FILE_SIZE = Number.parseInt(
  process.env.MAX_PDF_OCR_FILE_SIZE || String(8 * 1024 * 1024),
  10,
);
...
async function extractPdfText(file, options) {
  ...
  try {
    ...
    if (Number.isFinite(MAX_PDF_OCR_FILE_SIZE) && file.size > MAX_PDF_OCR_FILE_SIZE) {
      return {
        supported: false,
        text: "",
        strategy: "pdf-scanned-too-large",
        documentLayout,
        message:
          "The PDF appears to be scanned or image-based and is too large to OCR safely. Send a searchable PDF or .docx file.",
      };
    }
...
async function ocrPdfPages(parser, options) {
  const screenshotResult = await parser.getScreenshot({
    ...
    imageBuffer: true,
    desiredWidth: Number.isFinite(PDF_OCR_DESIRED_WIDTH)
      ? PDF_OCR_DESIRED_WIDTH
      : 1200,
  });
